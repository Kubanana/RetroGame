...
void draw_sprite(uint32_t* frameBuffer, uint32_t* sprite, int x, int y, int scale);
void draw_player(uint32_t* frameBuffer, int x, int y, int scale, int direction, int frame);
void draw_tilemap(uint32_t* frameBuffer, uint32_t* sprite, int tilemap[MAP_HEIGHT][MAP_WIDTH]);
